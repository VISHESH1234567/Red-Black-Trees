...
public:
    ...
    Path* findPathrec(MetroStop* station, string destination, vector<MetroLine*> linesreached,int direction){
        if (station==nullptr) return nullptr;
        if (station->getStopName() == destination){
            Path* stations = new Path(); stations->addStop(station);
            return stations;
        }
        AVLNode* temp = tree->searchStop(station->getStopName());
        if (direction){
            Path* answer = findPathrec(station->getNextStop(),destination,linesreached,1);
            if (answer!=nullptr){
                answer->addStop(station);
                answer->setTotalFare(answer->getTotalFare()+station->getFare()-station->getNextStop()->getFare());
                return answer;
            }
        }
        else{
            Path* answer = findPathrec(station->getPrevStop(),destination,linesreached,0);
            if (answer!=nullptr){
                answer->addStop(station);
                answer->setTotalFare(answer->getTotalFare()+station->getPrevStop()->getFare()-station->getFare());
                return answer;
            }
        }
        for (int i=0;i<temp->getStops().size();i++){
            ...
            if (flag){
                ...
                Path* answer = findPathrec(temp->getStops()[i],destination, linesreached,0);
                if (answer!=nullptr){return answer;}
                answer = findPathrec(temp->getStops()[i],destination, linesreached,1);
                if (answer!=nullptr){return answer;}
    ...
    Path* findPath(std::string origin, std::string destination){
        ...
        for (int i=0;i<start->getStops().size();i++){
            Path* stations = new Path();
            vector<MetroLine*> linesreached;
            linesreached.push_back(start->getStops()[i]->getLine());
            Path* finalpath = findPathrec(start->getStops()[i],destination,linesreached,1);
            if (finalpath!=nullptr) return finalpath;
            finalpath = findPathrec(start->getStops()[i],destination,linesreached,0);
            if (finalpath!=nullptr) return finalpath;
        }
        return nullptr;
    }
...
int main() {
    ...
    Path* paths = pathFinders->findPath("Moti Nagar","Kalkaji Mandir");
    paths->printPath();
